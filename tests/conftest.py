...
def override_settings():
    ...
    os.environ.update({
        ...
        # Important for testcontainers in GitHub Actions
...
def postgres_container(override_settings):
    """
    Custom PostgreSQL setup that directly uses Docker instead of testcontainers
    readiness checks, which can be problematic in CI environments.
    """
    # Define PostgreSQL configuration
    POSTGRES_USER = "postgres"
    POSTGRES_PASSWORD = "postgres"
    POSTGRES_DB = "postgres"
    CONTAINER_NAME = f"test-postgres-{int(time.time())}"  # Unique name
    HOST_PORT = 5432
    ...
    if in_github:
        # Use a simple approach in GitHub Actions
        try:
            # Start a PostgreSQL container directly with Docker CLI
            os.system(f"docker run --name {CONTAINER_NAME} -e POSTGRES_USER={POSTGRES_USER} "
                      f"-e POSTGRES_PASSWORD={POSTGRES_PASSWORD} -e POSTGRES_HOST_AUTH_METHOD=trust "
                      f"-e POSTGRES_DB={POSTGRES_DB} -d -p {HOST_PORT}:5432 postgres:15-alpine")

            # Wait for PostgreSQL to start (simple wait)
            time.sleep(15)  # Give it 15 seconds to start

            # Use localhost for GitHub Actions
            db_url = f"postgresql://{POSTGRES_USER}:{POSTGRES_PASSWORD}@localhost:{HOST_PORT}/{POSTGRES_DB}"
            ...
            print(f"PostgreSQL container is ready at {db_url}")

            # Return a simple object with the necessary interface for tests
            class MockContainer:
            ...
            yield MockContainer(CONTAINER_NAME, db_url)

        except Exception as e:
            print(f"Failed to set up PostgreSQL container: {e}")
            # Try to clean up
