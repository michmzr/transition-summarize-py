...
async def test_summarize():
    ...
    summary = await summarize(text, SUMMARIZATION_TYPE.TLDR, LANG_CODE.ENGLISH)
    assert summary is not None
    assert len(summary) > 0
    assert "Fallout" in summary

@pytest.mark.integration_no_yt
async def test_summarize_with_empty_input():
    summary = await summarize("", SUMMARIZATION_TYPE.TLDR, LANG_CODE.ENGLISH)
    assert summary == ""
...
@pytest.mark.integration_no_yt
async def test_summarize_with_different_types():
    text = "This is a test text that needs to be summarized in different ways."

    tldr_summary = await summarize(text, SUMMARIZATION_TYPE.TLDR, LANG_CODE.ENGLISH)
    assert tldr_summary is not None
    assert len(tldr_summary) > 0

    concise_summary = await summarize(text, SUMMARIZATION_TYPE.CONCISE, LANG_CODE.ENGLISH)
    assert concise_summary is not None
    assert len(concise_summary) > 0

    detailed_summary = await summarize(text, SUMMARIZATION_TYPE.DETAILED, LANG_CODE.ENGLISH)
